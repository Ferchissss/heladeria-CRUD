<button onclick="closeModal('modalEditCategoria')" class="absolute top-2 right-2 text-gray-500 hover:text-gray-700 text-xl">&times;</button>
<h2 class="text-xl font-bold mb-4">Editar Categoría</h2>
<form action="{{ route('admin.categorias.update', $categoria->id) }}" method="POST" enctype="multipart/form-data" id="editCategoryForm">
    @csrf
    @method('PUT')
    ...
    <div class="space-y-4">
        ...
        <div class="mb-4">
            ...
            @if($categoria->imagen_url)
                <img src="{{ asset('storage/' . $categoria->imagen_url) }}" alt="Imagen actual" class="w-full h-48 object-cover rounded mb-2">
        ...
        <div>
            ...
            <textarea name="descripcion" class="w-full border border-gray-300 rounded px-3 py-2 text-sm min-h-[100px] focus:outline-none focus:ring-1 focus:ring-blue-500">{{ old('descripcion', $categoria->descripcion) }}</textarea>
            @error('descripcion')
                <span class="text-red-500 text-xs">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="number" name="orden_display" value="{{ old('orden_display', $categoria->orden_display) }}" 
            ...
            <label class="flex items-center cursor-pointer">
                <div class="relative">
                    <input type="checkbox" name="activa" value="1" 
                           class="sr-only" {{ old('activa', $categoria->activa) ? 'checked' : '' }}>
                    <div class="block w-10 h-6 rounded-full {{ old('activa', $categoria->activa) ? 'bg-blue-500' : 'bg-gray-300' }}"></div>
                    <div class="dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition {{ old('activa', $categoria->activa) ? 'transform translate-x-4' : '' }}"></div>
                </div>
    ...
    <div class="flex justify-end gap-2 mt-6">
        <button type="button" onclick="closeModal('modalEditCategoria')" 
